...
export function mapGalleryRow(row: Row): GalleryStone {
  ...
  return {
    ...
    publishState: row.publish_state === 'draft' ? 'draft' : 'published', createdAt: row.created_at || '', updatedAt: row.updated_at || row.created_at || '',
    images: images.map((image: Row, index: number) => ({ id: image.id, storageKey: image.storage_key, url: image.url, width: image.width, height: image.height, mimeType: image.mime_type, alt: image.alt_text || `${row.title} rough gemstone, view ${index + 1}`, sortOrder: image.sort_order, isPrimary: image.is_primary })),
  };
}
...
function localPreview(): GalleryStone[] {
  return manifest.records.map(record => ({
    ...
    publishState: 'published', createdAt: '', updatedAt: '',
...
export async function getGalleryStones(): Promise<GalleryStone[]> {
  ...
  const result = await supabase.from('listings').select('id,sku,title,slug,category,gemstone_family,origin,origin_locality,public_weight_label,description,educational_note,form,colour,status,position,publish_state,created_at,updated_at,listing_images(*)').eq('publish_state', 'published').in('status', ['available', 'sold']).order('position');
  if (result.error) {
...
export async function getGalleryStoneBySlug(slug: string): Promise<GalleryStone | null> {
  const stones = await getGalleryStones();
  return stones.find((stone) => stone.slug === slug && stone.publishState === 'published') ?? null;
}
